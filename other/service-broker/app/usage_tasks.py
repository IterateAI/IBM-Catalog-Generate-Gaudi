...
MEASURE_NAME = "HEALTHCARE_UNIT"
RETRY_COUNT = 3
RETRY_DELAY = 5  # seconds
IBMCLOUD_SERVICE_ID_API_KEY = os.getenv("IBMCLOUD_SERVICE_ID_API_KEY")
...
usage_celery.conf.update(
    ...
    # Beat schedule - runs on 1st day of month at 2:00 AM UTC
    beat_schedule={
...
def get_ibm_iam_token(iam_url: str = "https://iam.cloud.ibm.com/identity/token"):
    data = {
        ...
        "apikey": IBMCLOUD_SERVICE_ID_API_KEY,
    }
    headers = {
...
def get_active_instances() -> List[ServiceInstance]:
    ...
    try:
        instances = (
            ...
            .filter(
                ServiceInstance.operation == "provision",
                ServiceInstance.state == "succeeded",
            )
            .all()
        )
...
def send_usage_to_ibm(
    ...
    body = [
        {
            ...
            "end": end_ms,
            "measured_usage": [
                {"measure": str(MEASURE_NAME).upper(), "quantity": quantity}
            ],
        }
    ]
    ...
    for attempt in range(1, RETRY_COUNT + 1):
        try:
            logger.info(
                f"[Attempt {attempt}] Sending usage to IBM for instance {instance_id}"
            )
            ...
            logger.warning(
                f"[Attempt {attempt}] IBM API returned {resp.status_code}: {resp.text}"
            )
...
def collect_and_report_usage(self):
    ...
    try:
        ...
        for instance in active_instances:
            ...
            try:
                # Get usage data from instance (Get healthcare units from db)
                logger.info(f"Collecting usage for instance {instance.instance_id}")
                units_used = int(instance.healthcare_units)

                valid_units = range(1000, 15001, 1000)
                if units_used not in valid_units:
                    raise ValueError("Invalid healthcare units")

                instance_result["usage_collected"] = units_used

                # Send to IBM
                logger.info(
                    f"Reporting {units_used} healthcare units for instance {instance.instance_id}"
                )
                ...
                logger.error(
                    f"Failed to process instance {instance.instance_id}: {str(e)}"
                )
